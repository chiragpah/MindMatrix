...
const uploadCourse=(async(req,res,next)=>{
    ...
    //     const videoFile = file[i];
    //     const mycloud = await cloudinary.v2.uploader.upload(videoFile.path, {
    //         resource_type: "video",
    //         folder: "courses"
    //     });
    
    //     // Update the existing object in courseData array with the new videoUrl field
    //     if (i < data.courseData.length) {
    //         data.courseData[i].videoUrl = mycloud.secure_url;
    //     } else {
    //         // If the index exceeds the current length of courseData, create a new object
    //         data.courseData.push({ videoUrl: mycloud.secure_url });
    //     }
    // }
    ...
    for (let i = 0; i < files.length-1; i++) {
      ...
      if (file.mimetype.startsWith('video')) {
          ...
          // Update the existing object in courseData array with the new videoUrl field
          if (i < data.courseData.length) {
              data.courseData[i].videoUrl = {  public_id:mycloud.public_id,
              url:mycloud.secure_url}
          } else {
              // If the index exceeds the current length of courseData, create a new object
              data.courseData.push({ videoUrl: mycloud.secure_url });
...
module.exports = { getComments };
  ...
  const getAdminAllCourses = (async (req, res, next) => {
    console.log("we are inside the get admin all courses")
    try {
 ...
 module.exports={uploadCourse,editCourse,getSingleCourse,getAllCourses,getCourseByUser,addComment,addReply,addReview,getAdminAllCourses,deleteCourse,getComments,addReplyToReview,uploadVideo}